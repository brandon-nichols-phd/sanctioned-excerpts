...
import { TECH_DEBT, type ItemForTable, type PhaseKey } from './item'
...
const Items: FC = () => {
  ...
  const { itemsForTable, extraColumnsForTable, isLoading, toggleActiveStatus } = useItems()
  ...
  const { columnHeaders, tableFields, scopedSlots } = useMemo(() => {
    ...
    return {
      ...
      scopedSlots: extraColumnsEntries.reduce((accum, [phaseKey]) => {
        accum[phaseKey] = (row: ItemForTable) => <td className="text-center">{row[phaseKey as PhaseKey] ?? '-'}</td>
        return accum
      }, {} as Record<string, (row: ItemForTable) => JSX.Element>),
    }
  }, [extraColumnsForTable])
    ...
    <div className="mb-5">
      <CCardHeader>
        ...
        <div className="float-right display-inline-block mx-1">
          <Link to={{ pathname: '/labels/items/new' }}>
